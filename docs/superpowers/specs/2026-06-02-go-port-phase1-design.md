...
```
go.mod                              # module github.com/Havoc24k/postgres4all; go 1.25
...
Every generated **init SQL** file begins with the header `-- generated by postgres4all; do not edit`.
- `init/01-extensions.sql` — header + `CREATE EXTENSION IF NOT EXISTS` for the enabled set + languages
  (header-only when no extension is needed).
- `init/02-schema.sql` — header + enabled capability `schema.sql` (+ `seed.sql` if seeding) in canonical
  order (`timeseries` before `dashboards`), one blank separator line between fragments.
- `init/00-roles.sh` (mode `0755`) + `init/03-api-grants.sql` — only if `api`; the `GRANT SELECT ON
  <tables>` line is emitted only for enabled read-capabilities and **omitted entirely** when that set is
  empty.
- `init/04-meta.sql` — header + `p4a_meta.capabilities` + inserts (always).
- `docker-compose.yml` — `db` always; `postgrest` only if `api`; ports bound to `127.0.0.1` unless
  `publish_externally`.
- `.env` (mode `0600`) — `POSTGRES_*` always; `AUTHENTICATOR_PASSWORD`/`JWT_SECRET` if `api`; secrets from
  config, else generated with pinned sizes: `POSTGRES_PASSWORD=Hex(24)` (48 chars),
  `AUTHENTICATOR_PASSWORD=Hex(16)` (32), `JWT_SECRET=Hex(48)` (96). Init SQL files are `0644`.
...
- **`generate_test.go`** — golden-file: for representative configs (minimal `document_store`;
  `gis`-only base swap; `api`+`auth`; `api`-only with no read-caps; all-on), call `Generate` into a temp
  dir and compare the tree to `testdata/golden/<case>/`. A `-update` flag regenerates goldens. The
  **committed goldens are the sole source of truth**; tests are deterministic because every golden config
  sets fixed secrets.
- **Format-compatibility check** — a ONE-TIME, by-inspection developer check (NOT a CI byte diff) that the
  Go `.env` keys, compose services, and init filenames align with `setup.sh --dry-run`. The two generators
  intentionally differ on header text, Dockerfile line-wrapping, and random secret lengths, so bash is not
  an oracle.
